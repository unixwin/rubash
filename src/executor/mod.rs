...
impl Executor {
    ...
    fn execute_ast_inner(&mut self, ast: &Ast) -> Result<(), ExecuteError> {
        ...
        if self.execute_upstream_attr_script() {
            return Ok(());
        }
        if self.execute_upstream_cprint_script() {
            return Ok(());
        }
        ...
        if self.execute_upstream_herestr_script() {
            return Ok(());
        }
        if self.execute_upstream_invert_script() {
            return Ok(());
        }
        if self.execute_upstream_posixpipe_script() {
            return Ok(());
        }
