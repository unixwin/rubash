...
#[test]
fn test_for_loop_function_assignment_preserves_remaining_values() {
    let output_path = "target/rubash-ifs-posix-loop-output.txt";
    let _ = fs::remove_file(output_path);
    let input = format!(
        "probe() {{ label=$1; str=$2; IFS=' '; set x $str; printf '%s\n' \"n=$# first=$1\" >> {output_path}; }}; \
         for str in '' '-' 'a' 'a b'; do probe one \"$str\"; done"
    );
    let tokens = tokenize(&input);
    let ast = parse(&tokens);
    let mut executor = Executor::new();
